...
#
# Replaces the previous Redis + Celery queue layer.
# RunPod's serverless infrastructure now handles all queuing internally.
#
# This module submits jobs to the RunPod serverless endpoint and polls
# for results synchronously. The caller waits for the response.
#
# Flow:
#   1. submit_job() — sends request to RunPod, gets a job ID back
#   2. poll_job()   — polls status every POLL_INTERVAL seconds
#   3. Returns result when status is COMPLETED or raises on FAILED/timeout
# =============================================================================
...
# RunPod job terminal states
COMPLETED = "COMPLETED"
FAILED    = "FAILED"
CANCELLED = "CANCELLED"


async def submit_job(model: str, prompt: str) -> str:
    """
    Submit an inference job to the RunPod serverless endpoint.
    ...
    Returns:
        The RunPod job ID string.

    Raises:
        httpx.HTTPStatusError: If RunPod returns a non-200 response.
    """
    payload = {
        "input": {
            "model":  model,
    ...
    job_id = data.get("id")
    logger.info(f"[runpod] Job submitted: {job_id} | model={model}")
    return job_id
...
async def poll_job(job_id: str) -> dict:
    ...
    Polls every POLL_INTERVAL seconds up to POLL_TIMEOUT seconds.
    ...
    Raises:
        TimeoutError:   If the job does not complete within POLL_TIMEOUT.
        RuntimeError:   If the job fails or is cancelled.
    """
    headers = {"Authorization": f"Bearer {settings.RUNPOD_API_KEY}"}
    url     = f"{settings.RUNPOD_STATUS_URL}/{job_id}"
    start   = time.time()
...
def extract_text(result: dict) -> str:
    """
    Extract the generated text from a completed RunPod job result.
...
async def run_inference(model: str, prompt: str) -> str:
    """
    Full inference pipeline — submit job, poll for result, return text.
    ...
    """
    import asyncio  # imported here to avoid circular issues

    job_id = await submit_job(model, prompt)
